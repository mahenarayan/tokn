...
The goal of the next phase is to make Tokn useful on real production payloads and traces, while preserving the current product boundary: analyze and report, but do not modify files.
...
2. improve structured outputs
3. import real trace formats
4. add actionable guidance that does not rewrite input
...
Extend the analyzer to support richer production payload shapes:
...
- New adapters are covered by analyzer and fixture based tests.
...
- Keep this import diagnostic and avoid modifying source data.
...
Add recommendations that explain how engineers might reduce context pressure without rewriting their input.
...
- add fixture based regression tests in `src/test/fixtures.test.ts`
...
- Keep Tokn from modifying user files.
